...
public class ApplicationController implements Initializable {
	...
	private Dialog<NodeViewModel> createAddDialog() throws ApplicationException {
		...
		*/
		...
		return new Dialog<NodeViewModel>() { //TODO Skrócić metodę!!
			{				
				setTitle("Add new comment");
				setHeaderText("Fill the fields");
				...
				case "Comment":
					...
					setResultConverter(dialogButton -> 
							dialogButton == OKButtonType ? new CommentViewModel(comment.getText(), author.getText()) : null);
					break;
				case "Task":
					...
					datePicker.valueProperty().addListener((observer, oldValue, newValue) -> 
						loginButton.setDisable(newValue == null));
					getDialogPane().setContent(grid);
					Platform.runLater(() -> author.requestFocus());
					setResultConverter(dialogButton -> dialogButton == OKButtonType ? 
							new TaskViewModel(comment.getText(), author.getText(), 
							Date.from(datePicker.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant())) : null);
					break;
				case "Suggestion":
					TextArea suggestion = new TextArea();
					suggestion.setPromptText("Suggestion");
					suggestion.setWrapText(true);
					grid.add(new Label("Suggestion: "), 0, 2);
					grid.add(suggestion, 1, 2);
					getDialogPane().setContent(grid);
					Platform.runLater(() -> author.requestFocus());
					setResultConverter(dialogButton -> dialogButton == OKButtonType ?
							new SuggestionViewModel(comment.getText(), author.getText(), suggestion.getText()) : null);
					break;
				case "Survey":
					getDialogPane().setContent(grid);
					Platform.runLater(() -> author.requestFocus());
					setResultConverter(dialogButton -> 
							dialogButton == OKButtonType ? new SurveyViewModel(comment.getText(), author.getText()) : null);
					break;
				}
			}
		};
	...
	private String getChoice(){
		...
		ChoiceDialog<String> choiceDialog = new ChoiceDialog<>("Comment", choices);
		choiceDialog.setTitle("Add new comment");
		choiceDialog.setHeaderText("Choice dialog");
		choiceDialog.setContentText("Choose type of comment:");
